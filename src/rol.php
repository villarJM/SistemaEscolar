...
$usuarios = mysqli_query($conexion, "SELECT * FROM usuarios WHERE idusuario = $id");
$consulta = mysqli_query($conexion, "SELECT * FROM permisousuario WHERE idusuario = $id");
...
foreach ($consulta as $asignado) {
    $datos[$asignado['idpermiso']] = true;
}
if (isset($_POST['permisos'])) {
    $id_user = $_GET['id'];
    $permisos = $_POST['permisos'];
    mysqli_query($conexion, "DELETE FROM permisousuario WHERE idusuario = $id_user");
    if ($permisos != "") {
        foreach ($permisos as $permiso) {
            $sql = mysqli_query($conexion, "INSERT INTO permisousuario(idusuario, idpermiso) VALUES ($id_user,$permiso)");
            if ($sql == 1) {
                ...
                <form method="post" action="">
                    ...
                    <?php while ($row = mysqli_fetch_assoc($sqlpermisos)) { ?>
                        <div class="form-check form-check-inline m-4">
                            ...
                            <input id="permisos" type="checkbox" name="permisos[]" value="<?php echo $row['idpermiso']; ?>" 
                            <?php
